...
#define Delay_ns(__ns)                                                        \
  if ((unsigned long)(F_CPU / 1000000000.0 * __ns) !=                         \
      F_CPU / 1000000000.0 * __ns)                                            \
...
#if !defined(RW_LINE_IMPLEMENTED) || \
    (RW_LINE_IMPLEMENTED != 0 && RW_LINE_IMPLEMENTED != 1)
...
// PIN defines
#define lcd_db0_pin_get() \
  (((PIN(LCD_DB0_PORT) & _BV(LCD_DB0_PIN)) == 0) ? 0 : 1)
#define lcd_db1_pin_get() \
  (((PIN(LCD_DB1_PORT) & _BV(LCD_DB1_PIN)) == 0) ? 0 : 1)
#define lcd_db2_pin_get() \
  (((PIN(LCD_DB2_PORT) & _BV(LCD_DB2_PIN)) == 0) ? 0 : 1)
#define lcd_db3_pin_get() \
  (((PIN(LCD_DB3_PORT) & _BV(LCD_DB3_PIN)) == 0) ? 0 : 1)
#define lcd_db4_pin_get() \
  (((PIN(LCD_DB4_PORT) & _BV(LCD_DB4_PIN)) == 0) ? 0 : 1)
#define lcd_db5_pin_get() \
  (((PIN(LCD_DB5_PORT) & _BV(LCD_DB5_PIN)) == 0) ? 0 : 1)
#define lcd_db6_pin_get() \
  (((PIN(LCD_DB6_PORT) & _BV(LCD_DB6_PIN)) == 0) ? 0 : 1)
#define lcd_db7_pin_get() \
  (((PIN(LCD_DB7_PORT) & _BV(LCD_DB7_PIN)) == 0) ? 0 : 1)
...
uint8_t lcd_getc() { return lcd_read(1); }
...
Returns:  none
*************************************************************************/
void lcd_goto(uint8_t pos) { lcd_command((1 << LCD_DDRAM) + pos); }
...
Returns:  none
*************************************************************************/
void lcd_clrscr() { lcd_command(1 << LCD_CLR); }
...
Returns:  none
*************************************************************************/
void lcd_home() { lcd_command(1 << LCD_HOME); }
...
#if (LCD_DISPLAYS == 1)
  if (LCD_DISPLAY_LINES > 1) lcd_db7_port_high();
#else
  unsigned char c;
  switch (ActiveDisplay) {
    case 1:
...
#else
#if (LCD_DISPLAYS == 1)
  if (LCD_DISPLAY_LINES < 2) lcd_db3_port_low();
#else
  unsigned char c;
  switch (ActiveDisplay) {
    case 1:
...
#if (LCD_DISPLAYS > 1)
void lcd_use_display(int ADisplay) {
  if (ADisplay >= 1 && ADisplay <= LCD_DISPLAYS) ActiveDisplay = ADisplay;
}
#endif
